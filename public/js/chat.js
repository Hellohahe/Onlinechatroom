...
sendUser.addEventListener('click', () => {
  ...
  join.innerHTML = '<p>Welcome to the chatroom share your commment !<p>';
...
socket.on('user-connected', (username) => {
  displayMsg.innerHTML += `<p><strong>${username}</strong> joinned the chatroom!</p>`;
...
socket.on('is-typing', (username) => {
  typingLabel.innerHTML = `<p>${username} entering...</p>`;
  chatWindow.scrollTop = chatWindow.scrollHeight;
});
...
socket.on('user-disconnected', (username) => {
  if (username == null) {
    ...
    displayMsg.innerHTML += `<p><strong>${username}</strong> always left !</p>`;
    chatWindow.scrollTop = chatWindow.scrollHeight;
    msgSound.play();
  }
});
